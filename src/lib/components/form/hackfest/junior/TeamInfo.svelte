<script lang="ts">
	...
	import type { JuniorHackfestRegistrationSchema } from '$lib/types/hackfest';

	const form = useFormContext<JuniorHackfestRegistrationSchema>();
		...
		<Form.Field {form} name="coachFirstName">
			<Form.Control>
				{#snippet children({ props })}
					...
					<Input
						{...props}
						bind:value={$formData.coachFirstName}
		...
		<Form.Field {form} name="coachLastName" class="mt-[21.8px]">
			<Form.Control>
				{#snippet children({ props })}
					<Input
						{...props}
						bind:value={$formData.coachLastName}
