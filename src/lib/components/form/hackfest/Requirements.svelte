<script lang="ts">
	...
	import type { AnyHackfestRegistrationSchema } from '$lib/types/hackfest';

	let { variant = 'junior' } = $props<{ variant?: 'junior' | 'senior' }>();

	const form = useFormContext<AnyHackfestRegistrationSchema>();
	...
	>
			...
			<div class="grid gap-6">
				<div class="block space-y-2">
					...
					<ul class="list-disc space-y-1 pl-5 text-muted-foreground">
						<li>
							A valid school ID or any official document serving as proof of
							enrollment
						</li>
					</ul>
				</div>
				<div class="space-y-2">
					...
					<ul class="list-disc space-y-1 pl-5 text-muted-foreground">
						<li>
							Proof that the coach is a bona fide teacher at the same school as the
							team members (e.g., school ID)
						</li>
					</ul>
				</div>
			</div>
		{:else}
			<p class="font-medium text-foreground">
				Compile all required documents (ID, proof of enrollment, etc.) of each member into a
				single <span class="font-mono text-purple-600 dark:text-purple-400">.zip</span> file for
				the entire team.
			</p>
		{/if}
	</GlassCard>
				...
				<div class="space-y-3">
					<div class="flex items-center justify-between">
						...
						{#if $errors.requirementsZip && !$formData.requirementsZip}
							<span class="animate-pulse text-xs font-medium text-red-500"
								>Required *</span
							>
						{/if}
					</div>
							...
							{:else}
								...
								>
									Click to upload <span
										class="font-mono text-purple-600 dark:text-purple-400"
										>.zip</span
									> file
								</p>
							{/if}
