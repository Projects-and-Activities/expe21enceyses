<script lang="ts">
	...
	import {
		...
		type SeniorHackfestRegistrationSchema
	} from '$lib/types/hackfest';

	let { data } = $props();

	const stepItems: StepItem<SeniorHackfestRegistrationSchema>[] = [
		{
			label: 'Team Info',
			component: TeamInfo,
			keys: ['teamName']
...
<div class="pointer-events-none fixed inset-0 -z-10 overflow-hidden select-none">
	<Star
		class="absolute -top-10 -right-10 w-56 -rotate-12 opacity-15 blur-[2px] dark:opacity-30"
	/>
	...
	<Star
		class="absolute bottom-20 -left-12 w-40 rotate-12 opacity-20 blur-[1px] dark:opacity-50"
	/>
